...
# %%
# Determine the detection parameters
scan_shape = dp_data.data.shape[:2]
...
datacube = dp_data.data[:, :, crop_ind[0]:crop_ind[1], crop_ind[2]:crop_ind[3]]
...
rys = np.random.randint(0, scan_shape[0], size=vir_y)
rxs = np.random.randint(0, scan_shape[1], size=vir_x)
